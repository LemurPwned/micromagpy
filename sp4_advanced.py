...
def set_n_demag(c, permute, func):
    ...
    while not it.finished:
        ...
        for i in np.rollaxis(np.indices((2,) * 6), 0, 7).reshape(64, 6):
            idx = tuple(
                map(lambda k: (it.multi_index[k] + n[k]) % (2 * n[k]) - n[k], range(3))
            )
            value += (-1) ** sum(i) * func(
                tuple(map(lambda j: (idx[j] + i[j] - i[j + 3]) * dx[j], permute))
            )
...
def h_eff(m):
    # demag field
    m_pad[: n[0], : n[1], : n[2], :] = m
    f_m_pad = np.fft.rfftn(m_pad, axes=tuple(filter(lambda i: n[i] > 1, range(3))))
...
f_n_demag = np.fft.rfftn(n_demag, axes=tuple(filter(lambda i: n[i] > 1, range(3))))
